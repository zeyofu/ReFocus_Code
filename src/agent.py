...
class SketchpadUserAgent(MultimodalUserProxyAgent):
    ...
    def receive(
        ...
        if self.executor:
            ...
            reply = self.prompt_generator.get_exec_feedback(exit_code, output, file_paths)
            # if execution fails
            if exit_code != 0:
                if self.sender_hits_max_reply(sender):
                    # reset the consecutive_auto_reply_counter
    ...
    def generate_init_message(self, ex, n_image):
        content = self.prompt_generator.initial_prompt(ex, n_image)
        return content

    def initiate_chat(self, assistant, ex, n_image, task_id, log_prompt_only=False):
        self.current_task_id = task_id
        self.feedback_types = []
        initial_message = self.generate_init_message(ex, n_image)
        if log_prompt_only:
